document.addEventListener('DOMContentLoaded', function() {
  ...
  const priceRanges = {
    ...
    'Telegram-бот': { min: 400, max: 400 },
    'Размещение сайта': { min: 115, max: 115 }
  };
  ...
  function updateAddButtons() {
    document.querySelectorAll('.add-btn').forEach(btn => {
      ...
      if (optionSelect) {
        itemName = optionSelect.options[optionSelect.selectedIndex].text.split(' (')[0];
      }
    ...
    if (!floatingCartBtn) {
      ...
      floatingCartBtn.innerHTML = `
        <div class="cart-icon">
          ...
          <span class="cart-badge">${cart.items.reduce((sum, item) => sum + item.quantity, 0)}</span>
        </div>
      `;
  ...
  function updateCart() {
    ...
    cart.items.forEach(item => {
      ...
      if (priceRanges[item.name]) {
        ...
        priceDisplay = `${range.min*item.quantity}-${range.max*item.quantity}₽`;
      }
      ...
      if (item.name === 'Карточки товаров' || item.name === 'Смысловой блок') {
        itemEl.querySelector('.cart-item-decrease').addEventListener('click', () => {
          updateCartItemQuantity(item.name, -1);
        });
        
        itemEl.querySelector('.cart-item-increase').addEventListener('click', () => {
          updateCartItemQuantity(item.name, 1);
        });
      }
    ...
    siteTotalEl.textContent = siteMinTotal === siteMaxTotal ? 
      `${siteMinTotal}₽` : 
    ...
    grandTotalEl.textContent = cart.minTotal === cart.maxTotal ? 
      `${cart.minTotal}₽` : 
  ...
  checkoutBtn.addEventListener('click', function() {
    ...
    const totalText = cart.minTotal === cart.maxTotal ? 
      `${cart.minTotal}₽` : 
      `${cart.minTotal}-${cart.maxTotal}₽`;
